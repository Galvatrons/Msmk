<template>
  <div class="Main-body">
    <div class="Main-body-l">
      <!-- 背景图头部 -->
      <div class="Main-box"></div>
      <div class="Main-top">
        <div class="top1" @click="lwh_MyInfo()">
          <van-image  v-if="lwh_token==''"
            class="lwh_img_head"
            round
            width="0.58rem"
            height="0.58rem"
            :src="lwh_head_cont.avatar"
          />
          <van-image v-if="lwh_token !=''"
            class="lwh_img_head"
            round
            width="0.58rem"
            height="0.58rem"
            :src="lwh_head_cont.avatar"
          />
          <div class="top-div">
            <span>{{ lwh_head_cont.nickname }}</span>
            <p><van-icon size="18" color="#f2995a" name="edit" /></p>
          </div>
          <div class="lwh_text" @click.stop="onClickyk">去约课</div>
        </div>
        <div class="top2">
          <div @click.stop="onClickfeature">
            <span>{{ lwh_head_list.courses }}</span>
            <p>我的特色课</p>
            <p style="color: gray">已购特色课程的学习</p>
          </div>
          <p></p>
          <div @click="onClickcoach">
            <span>{{ lwh_head_list.oto }}</span>
            <p>一对一辅导</p>
            <p style="color: gray">我的一对一老师辅导</p>
          </div>
          <p></p>
          <div @click="onClickstudy">
            <span>{{ lwh_head_list.integral }}</span>
            <p>剩余学习币</p>
            <p style="color: gray">查看剩余学习币</p>
          </div>
        </div>
        <!-- 去约课小图标 -->
      </div>
      <!-- 结束 -->
      <!-- 邀请好友注册app -->
      <div class="Main-app" @click="popup">
        <div class="Main-app-img">
          <img
            src="data:image/png;base64,"
            alt=""
          />
        </div>
        <div class="Main-app-text">
          <p>邀请好友注册APP，享多重好礼<br />现时特惠，多邀多得</p>
        </div>
        <div class="Main-app-left">
          <van-icon name="arrow" />
        </div>
      </div>
      <!-- 弹出层 -->
      <van-popup v-model="show"
        ><img
          class="popus"
          src="https://msmk2019.oss-cn-shanghai.aliyuncs.com/uploads/image/2020OPv5ksBfFx1603875973.png"
          alt=""
      /></van-popup>

      <!-- 结束 -->
      <!-- 课程相关 -->
      <div class="Main-k">
        <div class="list_title">
          <p class="lwh_text_p">课程相关</p>
        </div>
        <van-grid icon-size="24" :border="false">
          <van-grid-item
            icon="data:image/png;base64,"
            text="关注的老师"
            @click="onlickcontact"
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="我的收藏"
            @click="onlicksc"
          />
        </van-grid>
        <p class="lwh_bor_p"></p>

        <!-- 订单相关 -->
        <div class="list_title">
          <p class="lwh_text_p">订单相关</p>
        </div>
        <van-grid icon-size="24" :border="false">
          <van-grid-item
            icon="data:image/png;base64,"
            text="课程订单"
            @click="onlickKc"
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="会员订单"
            @click="onlickHy"
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="约客订单"
            @click="onlickYk"
          />
        </van-grid>
        <p class="lwh_bor_p"></p>
        <!-- 我的账户 -->
        <div class="list_title">
          <p class="lwh_text_p">我的账户</p>
        </div>
        <van-grid icon-size="24" :border="false">
          <van-grid-item
            icon="data:image/png;base64,"
            text="优惠卷"
            @click="onDiscount"
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="学习卡"
            @click="toCard()"
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="会员"
            @click='toVip()' 
          />
        </van-grid>
        <p class="lwh_bor_p"></p>

        <!-- 自助服务 -->
        <div class="list_title">
          <p class="lwh_text_p">自助服务</p>
        </div>
        <van-grid icon-size="24" :border="false">
          <van-grid-item
            icon="data:image/png;base64,"
            text="我的消息"
             @click='toMessage()'
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="意见反馈"
             @click='toFeedback()'
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="在线客服"
             @click="showLeave()"
          />
          <van-grid-item
            icon="data:image/png;base64,"
            text="设置"
            @click="onSetting"
          />
        </van-grid>
      </div>
    </div>
  </div>
</template>

<script>
import BetterScroll from "better-scroll";

export default {
  data() {
    return {
      lwh_bs: null,
      show: false,
      lwh_head_cont: [],
      lwh_head_list: [],
      lwh_token:window.localStorage.getItem("token") || ""
    };
  },
  mounted() {
    // 滑动插件
    this.$nextTick(() => {
      this.lwh_bs = new BetterScroll(".Main-body", {
        probeType: 3,
        click: true,
      });
      // 上滑禁止
      this.lwh_bs.on("scroll", (pos) => {
        if (pos.y >= 0) {
          // console.log(pos.y);
          this.lwh_bs.scrollTo(0, 0);
        }
      });
    });
    this.lwh_Ajax();
    this.lwh_AjaxInfo();
  },
  methods: {
    // 数据获取
    async lwh_Ajax() {
      let { data } = await this.$http.get(
        "https://www.365msmk.com/api/app/userInfo?"
      );
      this.lwh_head_cont = data.data;
      console.log(this.lwh_head_cont);
    },
    // 特色课数据获取
    async lwh_AjaxInfo() {
      let { data } = await this.$http.get(
        "https://www.365msmk.com/api/app/getUCenterInfo?"
      );
      this.lwh_head_list = data.data;
      console.log(this.lwh_head_list);
    },
    // 个人信息跳转
    lwh_MyInfo() {
      this.$router.push({
        path: "/lwh_myInfo",
      });
    },
    // 优惠券跳转
    onDiscount(){
      this.$router.push("/yzMydiscount")
    },
    onClickyk() {
      this.$router.push({
        path: "/oto",
      });
    },
    onClickstudy() {
      this.$router.push({
        path: "/Wyl-my-currency",
      });
    },
    onClickcoach() {
      this.$router.push({
        path: "/lwh_york",
      });
    },
    onClickfeature() {
      this.$router.push({
        path: "/Wyl-my-study",
      });
    },
    popup() {
      this.show = true;
    },
    onlickcontact() {
      this.$router.push({
        path: "/Wyl-concern",
      });
    },
    onlicksc() {
      this.$router.push({
        path: "/Wyl-collect",
      });
    },
    //课程跳转
    onlickKc() {
      this.$router.push({
        path: "/Wyl-Kc",
      });
    },
    //会员跳转
    onlickHy() {
      this.$router.push({
        path: "/Wyl-Hy",
      });
    },
    //约客跳转
    onlickYk() {
      this.$router.push({
        path: "/Wyl-Yk",
      });
    },
    // 设置跳转
    onSetting() {
      this.$router.push("/yzSetting");
    },
    // 显示留言组件
    showLeave(){
      this.$store.commit("wsyShowLeave",true)
    },
    // 跳转到信息反馈
    toFeedback(){
      this.$router.push("/wsyFeedback")
    },
    // 跳转到我的通知
    toMessage(){
      this.$router.push("/message")
    },
    // 跳转到会员购买页面
    toVip(){
      this.$router.push("/vip")
    },
    // 跳到学习卡页面
    toCard(){
      this.$router.push("/card")
    }
  },
  computed: {},
};
</script>

<style scoped lang='scss'>
.Main-body {
  width: 100%;
  height: 100%;
}
.Main-body-l {
  width: 100%;
  min-height: 101%;
}
.Main-box {
  width: 100vw;
  height: 1.2rem;
  background: rgb(255, 81, 0);
  border-radius: 0px 0px 0.7rem 0.7rem;
}
.Main-top {
  width: 3.6rem;
  height: 1.8rem;
  position: relative;
  /* background: yellow; */
  top: -0.8rem;
  left: 0.2rem;
  box-shadow: 0 0.01rem 0.06rem 0 rgba(0, 0, 0, 0.05);
  border-radius: 0.05rem;
  background: #fff;
  overflow: hidden;
}

.Main-top .top1 {
  width: 100%;
  height: 50%;
  background: rgb(255, 255, 255);
}
.top1 {
  padding: 0.15rem 0 0 0.15rem;
  box-sizing: border-box;
  display: flex;
  align-items: center;
  position: relative;
  > .top-div {
    display: flex;
    align-items: center;
    font-size: 0.16rem;
    > span {
      margin: 0 0.1rem 0 0.1rem;
    }
    > p {
      width: 0.15rem;
      height: 0.15rem;
    }
  }
  > .lwh_text {
    width: 0.55rem;
    height: 0.24rem;
    background-color: rgb(255, 81, 0);
    text-align: center;
    line-height: 0.24rem;
    color: #fff;
    position: absolute;
    top: 0.4rem;
    right: 0;
    font-size: 0.14rem;
    border-radius: 0.1rem 0 0 0.1rem;
  }
}
.Main-top {
  font-size: 0.2rem;
}
.Main-top .top2 {
  width: 100%;
  height: 50%;
  display: flex;
  justify-content: space-between;
  align-items: center;
}
.top2 {
  > p {
    width: 0.01rem;
    height: 0.54rem;
    background-color: #f5f5f5;
  }
  > div {
    width: 32%;
    text-align: center;
    height: 0.65rem;
    margin-top: 0.14rem;
    align-items: center;
    > span {
      font-size: 0.26rem;
      color: rgb(255, 81, 0);
    }
    > p:nth-of-type(1) {
      color: #595959;
    }
    > p:nth-of-type(2) {
      font-size: 0.08rem;
      color: #b7b7b7;
    }
  }
}
.Main-top .top2 p {
  font-size: 12px;
}

.Main-app {
  width: 3.43rem;
  height: 0.43rem;
  background-image: linear-gradient(45deg, rgb(253, 88, 3), rgb(255, 146, 71));
  border-radius: 0.5rem;
  margin: 0 auto;
  position: relative;
  top: -0.5rem;
  display: flex;
  align-items: center;
  justify-content: space-between;
}
.Main-app-img {
  width: 0.2rem;
  height: 0.17rem;
  float: left;
  margin: 0 0.1rem 0;
}
.Main-app-img img {
  width: 100%;
  height: 100%;
}
.Main-app-text {
  width: 70%;
  float: left;
}
.Main-app-text p {
  color: #fff;
  font-size: 0.14rem;
  margin-top: 1.5px;
}
.lwh_text_p {
  font-size: 0.16rem;
}
.Main-app-left {
  width: 15%;
  height: 0.3rem;
  font-size: 0.2rem;
  display: flex;
  justify-content: center;
  align-items: center;
  color: #fff;
  float: left;
}
.lwh_bor_p {
  width: 92%;
  height: 0.01rem;
  background: #eee;
  margin-left: 0.15rem;
}
.Main-k {
  width: 100%;
  position: relative;
  top: -0.5rem;
}
.list_title {
  width: 100%;
}
.list_title p {
  width: 90%;
  margin-left: 0.2rem;
  margin-top: 0.1rem;
}
.popus {
  width: 3.2rem;
  height: 6rem;
}
.lwh_img_head {
  border: 0.02rem solid #bdcdf1;
  background: #bdcdf1;
}
</style>
